...
def get_ledger_statement(company, account, from_date, to_date):
    ...
    Includes opening balance, all GL Entries with running balance, closing balance.
    Traces Payment Entry / Journal Entry back to their Dux Voucher source (PV/RV).
    """
    ...
    # ── Opening Balance ──────────────────────────────────────────────────────────
    # Sum of all GL entries for this account BEFORE the from_date
    ...
    # ob_net > 0  → debit balance (normal for assets/expenses)
    # ob_net < 0  → credit balance (normal for liabilities/income)

    # ── Period GL Entries ────────────────────────────────────────────────────────
    # JOIN Payment Entry / Journal Entry to recover the Dux Voucher source fields
    ...
    for e in entries:
        ...
        # Contra party: prefer the party name (readable), fall back to 'against' field
        if e.party:
            contra = e.party
        else:
            contra = _clean_against(e.against or "")

        # To/By rule (ICAI standard):
        #   account DEBITED  → "To [contra]"   (Dr side of the T-account)
        #   account CREDITED → "By [contra]"   (Cr side of the T-account)
...
def _resolve_voucher(e):
    """
    Map a GL Entry row to:
      - display_type  : user-facing voucher type label
      - display_vch   : voucher number to show (Dux Voucher if available, else backend doc)
      - vch_url       : URL to open the voucher on the Frappe desk
    """
    ...
    if src_dt == "Payment Voucher" and src_vch:
        ...
        return label, src_vch, f"/desk/payment-voucher/{src_vch}"

    if src_dt == "Receipt Voucher" and src_vch:
        return "Receipt Voucher", src_vch, f"/desk/receipt-voucher/{src_vch}"

    # ── Native ERPNext vouchers ───────────────────────────────────────
    if vt == "Journal Entry":
        return "Journal Entry", vno, f"/desk/journal-entry/{vno}"
    ...
    if vt == "Payment Entry":
        if pmt == "Internal Transfer":
            ...
            label = "Payment Entry"
        return label, vno, f"/desk/payment-entry/{vno}"

    # ── Fallback (Sales Invoice, Purchase Invoice, etc.) ─────────────
    slug = vt.lower().replace(" ", "-")
    return vt, vno, f"/desk/{slug}/{vno}"


def _clean_against(against_str):
    """
    Clean the GL Entry 'against' field.
    Returns the single account name, or 'Various' if comma-separated multiple.
    """
...
@frappe.whitelist()
def get_accounts_for_company(company, search_txt=""):
    """
    Return leaf accounts for a company matching the search text.
    Used by the account type-ahead picker in the ledger page.
    """
    if not company:
        return []

    like = f"%{search_txt}%" if search_txt else "%"
