...
@frappe.whitelist()
def export_party_trial_balance_xlsx(company, party_type, from_date, to_date, show_zero=0):
    from dux_voucher.dux_voucher.api.reports_api import get_party_trial_balance
    d = get_party_trial_balance(company, party_type, from_date, to_date,
                                 show_zero=show_zero)
    wb = _build_party_tb_workbook(d)
    fname = f"PartyTrialBalance_{_safe(party_type)}_{_safe(company)}_{from_date}_to_{to_date}.xlsx"
    _stream(wb, fname)
...
def _build_party_tb_workbook(d):
    """Party Trial Balance — Opening Dr/Cr, Debit, Credit, Closing Dr/Cr
    per party, split into proper Dr/Cr columns so the sheet sums cleanly."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Party Trial Balance"

    cols = [
        ("Party",       24, "left",  None),
        ("Name",        30, "left",  None),
        ("Opening Dr",  15, "right", "amt"),
        ("Opening Cr",  15, "right", "amt"),
        ("Debit (₹)",   15, "right", "amt"),
        ("Credit (₹)",  15, "right", "amt"),
        ("Closing Dr",  15, "right", "amt"),
        ("Closing Cr",  15, "right", "amt"),
    ]
    n_cols = len(cols)

    n = d["row_count"]
    sub = f"Party Trial Balance  ·  {d['party_type']}"
    period = (f"{d['from_date']}   →   {d['to_date']}    ·    "
              f"{n} part{'ies' if n != 1 else 'y'}")

    header_row = _write_title_block(ws, d["company"], sub, period, n_cols, ACCENT_LEDGER)
    _write_header_row(ws, [c[0] for c in cols], header_row, aligns=[c[2] for c in cols])
    _apply_widths(ws, [c[1] for c in cols])
    ws.freeze_panes = f"A{header_row + 1}"
    ...
    for i, r in enumerate(d["rows"]):
        zebra = (i % 2 == 1)
        od = flt(r["opening"]) if r["opening_type"] == "Dr" else 0
        oc = flt(r["opening"]) if r["opening_type"] == "Cr" else 0
        cd = flt(r["closing"]) if r["closing_type"] == "Dr" else 0
        cc = flt(r["closing"]) if r["closing_type"] == "Cr" else 0
        vals = [r["party"], r["party_name"],
                od or None, oc or None,
                flt(r["debit"]) or None, flt(r["credit"]) or None,
                cd or None, cc or None]
        for ci, v in enumerate(vals, start=1):
            cell = ws.cell(row=row, column=ci, value=v)
            cell.font = Font(name="Calibri", size=10)
            cell.alignment = Alignment(horizontal=cols[ci - 1][2],
                                         vertical="top", wrap_text=(ci == 2))
            cell.border = Border(bottom=HAIR)
            if zebra:
                cell.fill = PatternFill("solid", fgColor=ZEBRA_BG)
            if cols[ci - 1][3] == "amt":
                cell.number_format = NUM_FMT
        for ci, color in [(3, DR_RED), (5, DR_RED), (7, DR_RED),
                           (4, CR_GREEN), (6, CR_GREEN), (8, CR_GREEN)]:
    ...
    lbl = ws.cell(row=row, column=1,
                  value=f"TOTAL — {n} part{'ies' if n != 1 else 'y'}")
    lbl.font = Font(name="Calibri", size=10, bold=True, color=GRAY_TXT)
    lbl.alignment = Alignment(horizontal="right", vertical="center")
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
    for ci, val, color in [
        (3, d["total_opening_debit"],  DR_RED),
        (4, d["total_opening_credit"], CR_GREEN),
        (5, d["total_debit"],          DR_RED),
        (6, d["total_credit"],         CR_GREEN),
        (7, d["total_closing_debit"],  DR_RED),
        (8, d["total_closing_credit"], CR_GREEN),
    ]:
        c = ws.cell(row=row, column=ci, value=flt(val))
