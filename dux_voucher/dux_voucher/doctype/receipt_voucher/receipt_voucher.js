...
frappe.ui.form.on("Receipt Voucher", {
    ...
    before_save(frm) {
        if (frm.doc.entry_mode === "Head-wise" || frm.doc.entry_mode === "Party + Head Entry") {
            ...
            const net = total_credit - total_debit;
            const currency = frappe.boot.sysdefaults.currency || "INR";
            const received_in = frm.doc.received_in_account || "";

            if (net > 0) {
                frappe.show_alert({
                    message: __(
                        "Head-wise Receipt Summary: Total Credit {0} | Total Debit {1} | Net amount of {2} will be debited to {3} on submission",
                        [
    ...
    amount(frm) {
        if (frm.doc.entry_mode === "Head-wise") {
            _rv_headwise_autofill_on_amount_change(frm);
        }
    },

    received_in_account(frm) {
        if (frm.doc.received_in_account && frm.doc.company) {
            frappe.db.get_value("Account", frm.doc.received_in_account, "company", function (r) {
...
frappe.ui.form.on("RV Party Row", {
    ...
    party(frm, cdt, cdn) {
        ...
        _rv_open_party_dialog(frm, cdt, cdn, typed);
    },
...
frappe.ui.form.on("RV Account Row", {

    account_rows_add(frm, cdt, cdn) {
        _rv_headwise_autofill_new_row(frm, cdt, cdn);
    },

    debit(frm, cdt, cdn) {
        _rv_headwise_balance_next_row(frm, cdt, cdn);
    ...
    account(frm, cdt, cdn) {
        ...
        if (frm.doc.company) {
            frappe.db.get_value("Account", row.account, "company", function (r) {
                if (r && r.company !== frm.doc.company) {
                    ...
                    _rv_fetch_account_balance(frm, cdt, cdn, row.account);
                }
            });
        }
    },
...
function _rv_open_party_dialog(frm, cdt, cdn, prefill_txt) {
    if (_rv_party_dialog_open) return;
    _rv_party_dialog_open = true;
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            },
            {
                ...
                fieldtype: "HTML",
                options: '<div class="rv-party-results" style="max-height:360px;overflow-y:auto;margin-top:8px;"><div style="padding:16px;text-align:center;color:var(--color-text-secondary);font-size:13px;">Start typing to search...</div></div>'
            }
        ]
    });
    ...
    const $container = dialog.$wrapper.find(".rv-party-results");
    let _selected_idx = -1;
    let _current_results = [];
    let _is_keyboard_nav = false;

    const _highlight_row = function (idx) {
        $container.find(".rv-result-row").each(function (i) {
            if (i === idx) {
    ...
    const _render = function (results) {
        ...
        results.forEach(function (r, idx) {
            ...
            const $row = $('<div class="rv-result-row" style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:0.5px solid var(--color-border-tertiary);cursor:pointer;"></div>');
                ...
                callback: function (res) {
                    if (res.message) {
                        ...
                        if (res.message.balance_type !== "Nil") {
                            const currency = frappe.boot.sysdefaults.currency || "INR";
                            const indicator = res.message.balance_type === "Cr" ? "orange" : "blue";
                            frappe.show_alert({
                                message: __(
    ...
    $input.on("keydown", function (e) {
        const rows = $container.find(".rv-result-row");
...
function _rv_fetch_account_balance(frm, cdt, cdn, account) {
    frappe.call({
...
function _rv_apply_entry_mode(frm) {
    const mode = frm.doc.entry_mode;
    const is_combined = mode === "Party + Head Entry";

    frm.toggle_reqd("mode_of_payment", true);
    frm.toggle_reqd("received_in_account", true);
    frm.toggle_reqd("amount", mode === "Party-wise");
    ...
    frm.set_df_property("amount", "description",
        (mode === "Head-wise" || is_combined)
            ? __("Auto-calculated: Total Credit minus Total Debit from table")
            : ""
    );
...
function _rv_apply_payment_method_labels(frm) {
...
function _rv_set_received_in_filter(frm) {
...
function _rv_set_cost_center_filter(frm) {
...
function _rv_set_project_filter(frm) {
...
function _rv_sum_party_rows(frm) {
    let total = 0;
    ...
    if (Math.abs(old_amount - total) > 0.005 && total > 0) {
        ...
        const currency = frappe.boot.sysdefaults.currency || "INR";
        frappe.show_alert({
            message: __("Total receipt amount updated to {0}", [format_currency(total, currency)]),
            indicator: "blue"
        }, 5);
    }
}

function _rv_headwise_autofill_on_amount_change(frm) {
    const rows = frm.doc.account_rows || [];
    if (!rows.length) return;
    const first = rows[0];
    if (!flt(first.debit) && !flt(first.credit)) {
        frappe.model.set_value(first.doctype, first.name, "credit", flt(frm.doc.amount));
        _rv_show_headwise_totals(frm);
    }
}

function _rv_headwise_autofill_new_row(frm, cdt, cdn) {
    ...
    let net_other = 0;
    rows.forEach(function (r) {
        if (r.name !== cdn) net_other += flt(r.credit) - flt(r.debit);
    });

    const remaining = amount - net_other;
    if (remaining > 0) {
        frappe.model.set_value(cdt, cdn, "credit", remaining);
        frappe.model.set_value(cdt, cdn, "debit", 0);
    } else if (remaining < 0) {
        frappe.model.set_value(cdt, cdn, "credit", 0);
        frappe.model.set_value(cdt, cdn, "debit", Math.abs(remaining));
    }
    _rv_show_headwise_totals(frm);
}

function _rv_headwise_balance_next_row(frm, cdt, cdn) {
    const amount = flt(frm.doc.amount);
    if (!amount) return;
    ...
    rows.forEach(function (r) {
        if (r.name !== next_row.name) net_except_next += flt(r.credit) - flt(r.debit);
    });

    const remaining = amount - net_except_next;
    if (remaining > 0) {
        frappe.model.set_value(next_row.doctype, next_row.name, "credit", remaining);
        ...
        frappe.model.set_value(next_row.doctype, next_row.name, "debit", Math.abs(remaining));
    } else {
        frappe.model.set_value(next_row.doctype, next_row.name, "credit", 0);
        frappe.model.set_value(next_row.doctype, next_row.name, "debit", 0);
    }
...
function _rv_show_headwise_totals(frm) {
    let total_debit = 0, total_credit = 0;
    ...
    const net = total_credit - total_debit;
    const currency = frappe.boot.sysdefaults.currency || "INR";
    const received_in = frm.doc.received_in_account || __("Bank/Cash Account");
    ...
    if (total_debit > 0 || total_credit > 0) {
        ...
        frm.dashboard.show_headline(
            ...
            ' &nbsp;|&nbsp; ' +
            '<b>' + __("Auto debit to") + ' <i>' + received_in + '</i>:</b> ' +
...
function _rv_set_account_row_filter(frm) {
...
// ---------------------------------------------------------------
// RV Combined Row — event handlers
// ---------------------------------------------------------------

let _rv_combined_party_dialog_open = false;

frappe.ui.form.on("RV Combined Row", {
    ...
    party(frm, cdt, cdn) {
        ...
        frappe.model.set_value(cdt, cdn, "current_balance", 0);
        frappe.model.set_value(cdt, cdn, "balance_type", "");
        _rv_open_combined_party_dialog(frm, cdt, cdn, typed);
    },
    ...
    combined_rows_add(frm, cdt, cdn) {
        _rv_combined_autofill_new_row(frm, cdt, cdn);
    },

    debit(frm, cdt, cdn) {
        _rv_combined_balance_next_row(frm, cdt, cdn);
        _rv_show_combined_totals(frm);
    },
...
// ---------------------------------------------------------------
// RV Combined party search dialog
// ---------------------------------------------------------------

function _rv_open_combined_party_dialog(frm, cdt, cdn, prefill_txt) {
    if (_rv_combined_party_dialog_open) return;
    _rv_combined_party_dialog_open = true;
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            },
            {
                fieldname: "results_html",
                fieldtype: "HTML",
                options: '<div class="rvcb-party-results" style="max-height:360px;overflow-y:auto;margin-top:8px;"><div style="padding:16px;text-align:center;color:var(--color-text-secondary);font-size:13px;">Start typing to search...</div></div>'
            }
        ]
    });
    ...
    const $input = dialog.fields_dict.search_txt.$input;
    const $container = dialog.$wrapper.find(".rvcb-party-results");
    let _selected_idx = -1;
    let _current_results = [];
    let _is_keyboard_nav = false;

    const _highlight_row = function (idx) {
        $container.find(".rvcb-result-row").each(function (i) {
            if (i === idx) {
                $(this).css({ "background": "#e8e8e8", "outline": "2px solid #888" });
            } else {
                $(this).css({ "background": "", "outline": "" });
    ...
    const _render = function (results) {
        ...
        results.forEach(function (r, idx) {
            const c = color_map[r.party_type] || { bg: "#f1efe8", color: "#5f5e5a" };
            const $row = $('<div class="rvcb-result-row" style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:0.5px solid var(--color-border-tertiary);cursor:pointer;transition:background 0.1s;"></div>');
    ...
    const _rv_select_combined_party = function (r) {
        const row = locals[cdt][cdn];
        row._set_by_dialog = true;
        frappe.model.set_value(cdt, cdn, "party", r.name);
        frappe.model.set_value(cdt, cdn, "party_type", r.party_type);
        ...
        // Focus the credit field after selection (receipts default to credit)
        setTimeout(function () {
            const grid = frm.fields_dict["combined_rows"].grid;
            const $credit_input = grid.wrapper.find('.grid-row[data-name="' + cdn + '"] [data-fieldname="credit"] input');
            if ($credit_input.length) $credit_input.focus();
        }, 300);
    };
    ...
    $input.on("keydown", function (e) {
        const rows = $container.find(".rvcb-result-row");
        if (!rows.length) return;
        ...
        if (e.which === 40) {
            ...
            e.preventDefault(); e.stopPropagation();
            if (_selected_idx >= 0 && _current_results[_selected_idx]) {
                _rv_select_combined_party(_current_results[_selected_idx]);
            }
        } else if (e.which === 27) {
            e.preventDefault(); e.stopPropagation();
            dialog.hide();
    ...
    dialog.onhide = function () {
        _rv_combined_party_dialog_open = false;
    };
...
// ---------------------------------------------------------------
// RV Combined mode — auto-balance helpers (credit-first for receipts)
// ---------------------------------------------------------------

function _rv_combined_autofill_new_row(frm, cdt, cdn) {
    const amount = flt(frm.doc.amount);
    if (!amount) return;
...
function _rv_combined_balance_next_row(frm, cdt, cdn) {
    const amount = flt(frm.doc.amount);
    if (!amount) return;
...
function _rv_show_combined_totals(frm) {
    if (frm.doc.entry_mode !== "Party + Head Entry") return;
