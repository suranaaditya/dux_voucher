...
class ReceiptVoucher(Document):
    ...
    def insert(self, *args, **kwargs):
        ...
        *before* Frappe runs its cancelled-link validation.
        ...
        now-cancelled JE / Payment Entry. ``Document.insert`` then runs
        ...
        ``backend_references`` from the JE / PE it posts, so the
    ...
    def _create_party_receipt_entries(self):
        ...
        for row in self.party_rows:
            ...
            # all that's needed for the receipt to be attributable.
            #
            # PER ROW, not per voucher — one voucher can collect from several
            # parties and only some of them belong to the project. Same
            # row-then-header idiom as the JE modes.
